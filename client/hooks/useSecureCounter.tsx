...
*/
import { SecureCounterAddresses } from "@/abi/SecureCounterAddresses";
import { SecureCounterABI } from "@/abi/SecureCounterABI";

export type ClearValueType = {
  handle: string;
  clear: string | bigint | boolean;
};
type SecureCounterInfoType = {
  abi: typeof SecureCounterABI.abi;
  address?: `0x${string}`;
  chainId?: number;
  chainName?: string;
};
...
/**
 * Resolves SecureCounter contract metadata for the given EVM `chainId`.
 *
 ...
 * @example
 * const { abi, address } = getSecureCounterByChainId(chainId);
 */
function getSecureCounterByChainId(
  chainId: number | undefined
): SecureCounterInfoType {
  if (!chainId) {
    return { abi: SecureCounterABI.abi };
  }

  const entry =
    SecureCounterAddresses[chainId.toString() as keyof typeof SecureCounterAddresses];

  if (!("address" in entry) || entry.address === ethers.ZeroAddress) {
    return { abi: SecureCounterABI.abi, chainId };
  }
  ...
  return {
    ...
    abi: SecureCounterABI.abi,
  };
}

/*
 * Main SecureCounter React component with 3 buttons
 *  - "Decrypt" button: allows you to decrypt the current SecureCounter count handle.
 *  - "Increment" button: allows you to increment the SecureCounter count handle using FHE operations.
 *  - "Decrement" button: allows you to decrement the SecureCounter count handle using FHE operations.
 */
export const useSecureCounter = (parameters: {
  ...
  const secureCounterRef = useRef<SecureCounterInfoType | undefined>(undefined);
  ...
  const fheCounter = useMemo(() => {
    const c = getSecureCounterByChainId(chainId);

    secureCounterRef.current = c;

    if (!c.address) {
      setMessage(`SecureCounter deployment not found for chainId=${chainId}.`);
    }
  ...
  const refreshCountHandle = useCallback(() => {
    console.log("[useSecureCounter] call refreshCountHandle()");
    if (isRefreshingRef.current) {
      return;
    }

    if (
      !secureCounterRef.current ||
      !secureCounterRef.current?.chainId ||
      !secureCounterRef.current?.address ||
      !ethersReadonlyProvider
    ) {
    ...
    const thisChainId = secureCounterRef.current.chainId;
    const thisFheCounterAddress = secureCounterRef.current.address;

    const thisFheCounterContract = new ethers.Contract(
      thisFheCounterAddress,
      secureCounterRef.current.abi,
      ethersReadonlyProvider
    );
    ...
    thisFheCounterContract
      ...
      .then((value) => {
        ...
        if (
          ...
          thisFheCounterAddress === secureCounterRef.current?.address
        ) {
      ...
      .catch((e) => {
        setMessage("SecureCounter.getCount() call failed! error=" + e);

...
const CONTRACTS = ["SecureCounter", "CipherBoard"];
...
function readDeployment(chainName, chainId, contractName, optional) {
  ...
  if (!fs.existsSync(chainDeploymentDir) || !fs.existsSync(contractFile)) {
    if (chainId === 31337 && !fs.existsSync(chainDeploymentDir)) {
      // Try to auto-deploy the contract on hardhat node!
      deployOnHardhatNode();
    }
    ...
    if (!fs.existsSync(chainDeploymentDir) || !fs.existsSync(contractFile)) {
      console.error(
        `${line}Unable to locate '${contractFile}' file.\n\n1. Goto '${dirname}' directory\n2. Run 'npx hardhat deploy --network ${chainName}'.${line}`
      );
      if (!optional) {
        process.exit(1);
      }
